...
main() {
  final keyBytes = [ 0xcc, 0xe8, 0xd3, 0x97, 0xf3,
                    0x16, 0x91, 0x59, 0x67, 0x77,
                    0x63, 0x97, 0xd7, 0x7c, 0x57,
                    0xb1, 0xff, 0xe4, 0x84, 0xd7,
                    0x94, 0x4a, 0x74, 0x3a, 0x41,
                    0x92, 0x28, 0x72, 0x4e, 0xb5,
                    0x99, 0x0f];
  ...
  group('EncryptedCommunicator', () {
    ...
    test("subscribe", () {
      ...
      final sentMessageStr = verify(_mockWebSocket!.add(captureAny)).captured.first;
    ...
    test("publish", () {
      ...
      final sentMessageStr = verify(_mockWebSocket!.add(captureAny)).captured.first;
      final sentMessage = Message.fromJson(jsonDecode(sentMessageStr));
      final encodedMessage = EncryptionPayload.fromJson(jsonDecode(sentMessage.payload));
      final String payload = encryptedPayloadHelper!.decryptAndVerify(encodedMessage);
    ...
    test('invalid json in message', () async {
      _communicator!.stream.listen((_) { });// stream transform will not fire without a listener
      ...
      expect(await completer!.future, predicate((arg) {
        ...
        expect(sentMessage.type, "pub");
        expect(payload, """{"jsonrpc":"2.0","id":null,"error":{"code":-32700,"message":"Parse error"}}""");
        return true;
      }));
    });
    ...
    test('invalid json in message payload', () async {
      _communicator!.stream.listen((_) { });// stream transform will not fire without a listener
      _remoteStreamController!.add("{\"topic\":\"Some Topic\",\"type\":\"pub\",\"payload\":\"Not Json\"}");
    ...
    test('invalid parameter - Not pub', () async {
      ...
      final sentMessage = Message.fromJson(jsonDecode(sentMessageStr));
      final encodedMessage = EncryptionPayload.fromJson(jsonDecode(sentMessage.payload));
      final String payloadStr = encryptedPayloadHelper!.decryptAndVerify(encodedMessage);

      expect(sentMessage.topic, "Topic");
      expect(sentMessage.type, "pub");
      expect(payloadStr, """{"jsonrpc":"2.0","id":null,"error":{"code":-32602,"message":"Invalid params"}}""");
    });

    test('invalid parameter - invalid encodable', () async {
      _communicator!.stream.listen((_) { });// stream transform will not fire without a listener
      final payload = <String,dynamic> {
        "A": "Not a valid message"
      };
