...
class Bech32 {
  String encode(String hrp, List<int> fiveBitData, String seperator) {
    ...
    return String.fromCharCodes([...hrp.codeUnits, ...seperator.codeUnits, ...chars ]);
  }
  ...
  Bech32Data? decode(String bech32) {
    ...
    if(!bech32.codeUnits.every((c) => c >= MIN_VALID_CODEPOINT && c <= MAX_VALID_CODEPOINT)) {
    ...
    final dataBytes = dataString.codeUnits.map((e) => _charset.indexOf(e)).toList();
    final fiveByteString = dataString.substring(0, dataString.length - CHECKSUM_SIZE);
    final checkSumString = dataString.substring(dataString.length - CHECKSUM_SIZE);

    final fiveBytes = fiveByteString.codeUnits.map((e) => _charset.indexOf(e)).toList();
    final checksum = checkSumString.codeUnits.map((e) => _charset.indexOf(e)).toList();
    ...
    return Bech32Data(hrp, dataBytes.sublist(0, dataBytes.length - CHECKSUM_SIZE));
  }

  List<int> _checksum(String hrp, List<int> data) {
    var values = [..._expandHrp(hrp), ...data, ...[0,0,0,0,0,0]];
